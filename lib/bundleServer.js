...
function setup (mode, cb) {
  ...
  this.bundleSserver = express()
  this.bundleSserver.use(express.static(process.cwd()))
  this.bundleSserver.use('/bundle.js', browserify(path.join(process.cwd(), this.args[0])))

  // Run the server on a random port that is then printed out to the console
  getPort().then(port => {
    this.bundleServerPort = port
    this.bundleSserver.listen(port, () => {
